...
export const addReview = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (course) {
      course.ratings = avg / course.reviews.length; // eg: we have 2 reviews one is 5 another one is 4 so => 9/2=4.5
    }

    await course?.save();
    await redis.set(courseId, JSON.stringify(course), "EX", 604800); //7 hari
